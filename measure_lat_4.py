...
def measure_gpu_memory_latency(data, iters, threads_per_block, blocks_per_grid):
...
def measure_latency_and_plot(iters):
    ...
    sizes = [100, 1000, 10000, 20000, 30000, 100000, 1000000, 1500000, 2000000, 3000000, 9000000, 12000000]
    latencies = []
    data_sizes = []

    threads_per_block = 256
    blocks_per_grid = 84  # Number of SMs
    ...
    for n in sizes:
        ...
        gpu_latency = measure_gpu_memory_latency(data, iters, threads_per_block, blocks_per_grid)
    ...
    plt.xscale('log')
    plt.yscale('log')
    plt.savefig('gpu_memory_latency.pdf')  # Save the figure to a file
    plt.show()
